...
Master.prototype.checkLocks = function(){
    ...
    this.redisClient.multi().
        select(this.db)["eval"](this.scripts.checkLocks.script, 5, this.prefix + "locks", this.prefix + "unassigned", this.prefix + "worker.", this.prefix + "job.", this.prefix + "workers", Date.now() - 60 * 1000).
        exec((function(err, replies){
            setTimeout(this.checkLocks.bind(this), 500);
        }).bind(this));
};
